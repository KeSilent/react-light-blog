...
// 渲染Model中的字段
func GenerateField(field request.AutoCodeField) string {
	// 构建gorm标签
	gormTag := ``
...
// 格式化搜索条件语句
func GenerateSearchConditions(fields []*request.AutoCodeField) string {
...
// 格式化前端搜索条件
func GenerateSearchFormItem(field request.AutoCodeField) string {
	// 开始构建表单项
...
func GenerateTableColumn(field request.AutoCodeField) string {
...
func GenerateFormItem(field request.AutoCodeField) string {
	// 开始构建表单项
	result := fmt.Sprintf(`<el-form-item label="%s:" prop="%s">
`, field.FieldDesc, field.FieldJson)
...
func GenerateDescriptionItem(field request.AutoCodeField) string {
	// 开始构建描述项
...
func GenerateDefaultFormValue(field request.AutoCodeField) string {
	// 根据字段类型确定默认值
...
func GenerateSearchField(field request.AutoCodeField) string {
	var result string
